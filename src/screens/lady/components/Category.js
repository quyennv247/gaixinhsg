...
import categoryService from '../../../api/categoryLadyService';
    ...
    renderItem(item){
        return(
            <Pressable key={item.Id} onPress={() => this.handleClickSearch(item.Id)} style={this.state.categoryId == item.Id ? styles.itemActive : styles.item}>
                <Text style={this.state.categoryId == item.Id ? styles.itemTextActive : styles.itemText}>{item.CategoryName}</Text>
            </Pressable>
        )
    }
}
